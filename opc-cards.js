...
(function(){
  ...
  try{ console.log("%c[ONLY POEMS] share-cards v14 loaded","color:#0153db;font-weight:bold"); }catch(e){}
  ...
  function init(){
    ...
    var _pinned=false;
    try{
      var _tt=document.querySelector(".random-poem, .tag-poems_heading");
      var _pp=document.querySelector(".random-poem-poet, .random-poet, .tags-poet-name");
      if(_tt){ var _tx=_tt.textContent.trim(); if(_tx && _tx.length<200){ info.title=_tx; _pinned=true; } }
      if(_pp){ var _px=_pp.textContent.trim(); if(_px && _px.length<100){ info.poet=_px; _pinned=true; } }
    }catch(_e){}
