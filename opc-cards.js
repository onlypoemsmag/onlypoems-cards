...
(function(){
  ...
  try{ console.log("%c[ONLY POEMS] share-cards v16 loaded","color:#0153db;font-weight:bold"); }catch(e){}
  ...
  function init(){
    ...
    // Interviews: the interviewed poet is the /poets/ link in the article header (the JSON-LD lacks it),
    // so attribute the shared quote to them instead of falling back to just the ONLY POEMS logo.
    if(isInterview){ try{
    ...
    // Random-poem & Tags pages: the poem TITLE and the POET name are BOTH headings, so the script
    // grabbed the wrong one and dropped the real title. Pin them to their CMS field classes.
    var _pinned=false;
    if(/random-poem/.test(path)||/\/(tags|read-poems-by-tags)\//.test(path)){ try{
      var _pp=document.querySelector(".random-poem-poet, .random-poet, .tags-poet-name");
      var _pn="";
      if(_pp){ _pn=_pp.textContent.trim(); if(_pn && _pn.length<100){ info.poet=_pn; _pinned=true; } }
      var _cand=[].slice.call(document.querySelectorAll(".tag-poems_heading, .author_poems_text, .random-poem"));
      for(var _k=0;_k<_cand.length;_k++){ var _c=_cand[_k];
        if(!_c.getClientRects().length || _c.closest('nav,.w-nav,footer,.footer,[class*="footer"]')) continue;
        if(_c===_pp || (_pp&&_c.contains(_pp)) || _c.matches(".random-poem-poet,.random-poet,.tags-poet-name")) continue;
        var _ct=_c.textContent.trim().replace(/\s+/g," ");
        if(_ct && _ct.length<200 && _ct.toLowerCase()!==_pn.toLowerCase()){ info.title=_ct; _pinned=true; break; }
      }
    }catch(_e){} }
    ...
    var POEM_SEL=".op-poem, .poem_text_potm_rich_text";   // tag the poem-text field with this class in Webflow → the whole feature binds to it exactly (bio / notes / caption ignored). Otherwise fall back to a heuristic.
    var poems=[].slice.call(document.querySelectorAll(POEM_SEL)).filter(function(el){return el.getClientRects().length&&isContent(el);});
    var _pure=/\/(poems-catalog|tags|read-poems-by-tags)\//.test(path)||/random-poem/.test(path);   // pure poem-list pages: every entry is a poem, so allow single-line (monostich) poems
    if(!poems.length) poems=list.filter(function(el){return el.textContent.trim().length>=(_pure?1:60) && !/\bnote\b/i.test(titleFor(el));});
